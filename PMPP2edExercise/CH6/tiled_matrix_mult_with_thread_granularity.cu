...
// This now has doubles the load from B and each thread calcs two elements in C
__global__ void matrix_multiplication_tiled_cuda(float *A, float *B, float *C,
  ...
  __shared__ float B_s[TILE_SIZE][TILE_SIZE * 2];
  ...
  for (int phase_number = 0; phase_number < dimension_size / TILE_SIZE;
    ...
    // we load from B twice as rarely
    int row_into_B = TILE_SIZE * phase_number + tile_row;
    int linearized_B_i1 = row_into_B * dimension_size + 2 * output_col;
    int linearized_B_i2 = row_into_B * dimension_size + 2 * output_col + 1;
    B_s[tile_row][2 * tile_col] = B[linearized_B_i1];
    B_s[tile_row][2 * tile_col + 1] = B[linearized_B_i2];
    __syncthreads();

    float local_result1 = 0;
    float local_result2 = 0;
    ...
    for (int reduction_axis = 0; reduction_axis < TILE_SIZE; reduction_axis++) {
      ...
      printf("Thread (%d, %d) in block (%d, %d) in phase %d "
             "has A_s = %f and B_s = %f with linearized_A_i = %d and lineared_B_i1 = %d\n",
             tile_col, tile_row, blockIdx.x, blockIdx.y, phase_number,
             A_s[tile_row][reduction_axis], B_s[reduction_axis][tile_col],
             linearized_A_i, linearized_B_i1);
      }
      local_result1 +=
          A_s[tile_row][reduction_axis] * B_s[reduction_axis][2 * tile_col];
      local_result2 += 
          A_s[tile_row][reduction_axis] * B_s[reduction_axis][2 * tile_col + 1];
    }

    C[output_row * dimension_size + 2 * output_col] += local_result1;
    C[output_row * dimension_size + 2 * output_col + 1] += local_result2;
    __syncthreads();
  }
}
...
void print_matrix(float *matrix, int dimension_size, int spacing = 5) {
  ...
  for (int i = 0; i < dimension_size; i++) {
    for (int j = 0; j < dimension_size; j++) {
      std::cout << std::scientific << std::setprecision(3) << std::setw(spacing) << " " << matrix[i * dimension_size + j];
    }
    std::cout << std::endl;
  }
...
std::pair<float *, double> run_code(float *h_A, float *h_B, int mode, int N) {
  ...
  case MODE_GPU_NAIVE:
    ...
    break;
  case MODE_GPU_TILED:
    dimGrid = dim3(grid_dim / 2, grid_dim);
